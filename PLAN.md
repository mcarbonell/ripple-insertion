...
**Status:** Early stage - extraction from k-alternatives repo **Goal:**
Standalone repo for the O(N log N) Dynamic TSP algorithm
...
- [ ] Create `data/` with demo subset (6 instances):
    - `eil51.json`, `berlin52.json`, `st70.json`, `kroA100.json`, `ch130.json`,
      `ch150.json`
...
- [ ] `package.json` with name `ripple-insertion`
- [ ] `.gitignore`
...
Both visualizers have the algorithm embedded inline. Extract into reusable
modules.
...
The full `tsplib-json/` directory has ~80 parsed TSPLIB instances (~2MB).
Copying it into every repo that uses TSP data is wasteful. Instead:
...
This algorithm was originally developed inside the k-alternatives repo as a
"bonus" experiment. It has been extracted because:
